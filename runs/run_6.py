...
#             valids = text_dis(to_one_hot(fake_texts, d_vocab))
            ...
            expected_reward = torch.mean(seq_log_probs * valids)
            ploss = pairwise_loss(noises, fake_text_features)
            text_gen_loss = -expected_reward + ploss
...
if __name__ == '__main__':
    run(config=dict(num_epochs=50), output_dir='run_6/')
