...
class BatchTextDiscriminator(nn.Module):
    def __init__(self, d_batch, d_text_feature, **kwargs):
        super().__init__()
    ...
    def define_module(self):
        self.batch_logit = nn.Linear(self.d_batch*self.d_text_feature, 1)

    def forward(self, text_features):
        batch_valid = self.batch_logit(text_features.view(1, -1)).squeeze(0)
        return batch_valid
...
def test_batch_text_disciminator():
    ...
    batch_text_dis = BatchTextDiscriminator(d_batch=d_batch, d_text_feature=d_text_feature).to(device).train()
...
def train(text_gen, text_dis, batch_text_dis, dataset, loader, device, output_dir, references=None, text_gen_opt_weights_path=None, text_dis_opt_weights_path=None,
    d_batch=512,
    num_epochs=20,
    gen_beta1=0.5,
    dis_beta1=0.5,
    ...
    **kwargs):
    ...
    text_gen.train()
    text_dis.train()
    batch_text_dis.train()

    criterion = nn.BCEWithLogitsLoss()

    real_targets = torch.ones(d_batch, device=device)
    fake_targets = torch.zeros(d_batch, device=device)
    batch_real_target = torch.ones(1, device=device)
    batch_fake_target = torch.zeros(1, device=device)

    text_gen_opt = optim.Adam(text_gen.parameters())
    text_dis_opt = optim.Adam(list(text_dis.parameters()) + list(batch_text_dis.parameters()))
...
#     text_dis_opt = optim.Adam(text_dis.parameters(), lr=dis_lr, betas=(dis_beta1, 0.999), weight_decay=dis_weight_decay)
    if text_gen_opt_weights_path:
        text_gen_opt.load_state_dict(torch.load(text_gen_opt_weights_path))
    if text_dis_opt_weights_path:
        text_dis_opt.load_state_dict(torch.load(text_dis_opt_weights_path))
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i, batch in tqdm(enumerate(loader, start=1), total=len(loader)):
            ...
            # train discriminator
            text_dis.zero_grad()
            text_features, valids = text_dis(to_one_hot(real_texts, d_vocab))
            batch_valid = batch_text_dis(text_features)
#             dbg1 = criterion(valids, real_targets)
#             dbg2 = criterion(batch_valid, batch_real_target)
#             print('criterion(valids, real_targets):', dbg1.item())
#             print('criterion(batch_valid, batch_real_target):', dbg2.item())
#             text_dis_real_loss = dbg1 + dbg2
#             text_dis_real_loss = criterion(valids, real_targets) + criterion(batch_valid, batch_real_target)
            text_dis_real_loss = criterion(valids, real_targets) + batch_div_importance * criterion(batch_valid, batch_real_target)
            text_dis_real_loss.backward()
            epoch_real_score += valids.mean().item()

            text_features, valids = text_dis(to_one_hot(fake_texts, d_vocab))
            batch_valid = batch_text_dis(text_features)
#             dbg1 = criterion(valids, fake_targets)
#             dbg2 = criterion(batch_valid, batch_fake_target)
#             print('criterion(valids, fake_targets):', dbg1.item())
#             print('criterion(batch_valid, batch_fake_target):', dbg2.item())
#             text_dis_fake_loss = dbg1 + dbg2
#             text_dis_fake_loss = criterion(valids, fake_targets) + criterion(batch_valid, batch_fake_target)
            text_dis_fake_loss = criterion(valids, fake_targets) + batch_div_importance * criterion(batch_valid, batch_fake_target)
...
#             expected_reward = torch.mean(seq_log_probs * rewards) * batch_valid.detach()
            expected_reward = torch.mean(seq_log_probs * rewards) + batch_div_importance * batch_valid.detach()
            text_gen_loss = -expected_reward
...
#     dataset, loader = get_emnlp_2017_news_pretrained_vocab(**config)
    ...
    batch_text_dis = BatchTextDiscriminator(d_batch=config['d_batch'], d_text_feature=d_text_feature)
    batch_text_dis.to(device)

    print('training:')
    train(text_gen, text_dis, batch_text_dis, dataset, loader, device, output_dir=output_dir, references=references, **config)
...
if __name__ == '__main__':
    run(config=dict(
        num_epochs=35,
        text_gen_weights_path='run_9_dbg/latest_text_gen.pth',
        text_dis_weights_path='run_9_dbg/latest_text_dis.pth',
        text_gen_opt_weights_path='run_9_dbg/latest_text_gen_opt.pth',
#         text_dis_opt_weights_path='run_9_dbg/latest_text_dis_opt.pth'
        )
        , output_dir='run_10/')
# # End
